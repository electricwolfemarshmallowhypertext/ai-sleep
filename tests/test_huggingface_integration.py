...
class TestHuggingFaceModelAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.adapter = HuggingFaceModelAdapter(
            model=None, model_id="test-model", enable_monitoring=True, auto_adapt=True
        )
    ...
    def test_initiate_light_sleep_with_enable(self):
        ...
        self.assertEqual(
            self.adapter.controller.model_state.current_mode, SleepMode.LIGHT_SLEEP
        )
    ...
    def test_initiate_deep_sleep_with_enable(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.adapter.controller.model_state.current_mode, SleepMode.DEEP_SLEEP
        )
    ...
    def test_wake_model(self):
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.adapter.controller.model_state.current_mode, SleepMode.AWAKE
        )

    def test_track_inference_metrics(self):
        """Test tracking inference metrics."""
        self.adapter.track_inference_metrics(
            perplexity=3.5, loss=0.5, inference_time=0.1, memory_usage=512.0
        )
...
class TestCreateSleepEnabledModel(unittest.TestCase):
    ...
    def test_create_without_loading(self):
        ...
        adapter = create_sleep_enabled_model(
            "gpt2", enable_monitoring=True, auto_adapt=True, load_model=False
        )
    ...
    def test_create_with_loading_mock(self):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            adapter = create_sleep_enabled_model("gpt2", load_model=True)
