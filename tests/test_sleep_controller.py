...
class TestSleepTrigger(unittest.TestCase):
    ...
    def test_triggers_exist(self):
        ...
        self.assertEqual(
            SleepTrigger.PERFORMANCE_DEGRADATION.value, "performance_degradation"
        )
...
class TestOptimizationStrategy(unittest.TestCase):
    ...
    def test_strategies_exist(self):
        ...
        self.assertEqual(
            OptimizationStrategy.GRADIENT_CLIPPING.value, "gradient_clipping"
        )
        self.assertEqual(
            OptimizationStrategy.ATTENTION_HEAD_PRUNING.value, "attention_head_pruning"
        )
        self.assertEqual(
            OptimizationStrategy.KV_CACHE_MANAGEMENT.value, "kv_cache_management"
        )
        self.assertEqual(
            OptimizationStrategy.SEMANTIC_CONSOLIDATION.value, "semantic_consolidation"
        )
        self.assertEqual(
            OptimizationStrategy.LAYER_NORM_RECALIBRATION.value,
            "layer_norm_recalibration",
        )
...
class TestAISleepController(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.controller = AISleepController(
            model_id="test-model", enable_monitoring=True, auto_adapt=True
        )
    ...
    def test_set_trigger_threshold(self):
        ...
        self.assertEqual(
            self.controller.trigger_thresholds[SleepTrigger.DRIFT_DETECTED], 0.01
        )
    ...
    def test_initiate_light_sleep(self):
        ...
        self.assertEqual(
            self.controller.model_state.current_mode, SleepMode.LIGHT_SLEEP
        )
    ...
    def test_gradient_clipping_optimization(self):
        ...
        self.controller.configure_light_sleep(
            strategies=[OptimizationStrategy.GRADIENT_CLIPPING], gradient_clip_norm=1.0
        )
    ...
    def test_kv_cache_management(self):
        ...
        for i in range(10):
            self.controller.model_state.update_kv_cache(
                f"layer_{i}", np.random.rand(10, 64), np.random.rand(10, 64)
            )
