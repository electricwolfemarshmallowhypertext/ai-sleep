...
if TORCH_AVAILABLE:
    from src.ai_sleep.c2c import KVSpec, KVCache, CacheProjector, LayerGate, C2CFuser
...
class TestC2CFuser(unittest.TestCase):
    ...
    def test_save_and_load_weights(self):
        ...
        try:
            ...
            self.assertTrue(torch.allclose(loaded_fuser.gate.alpha, fuser.gate.alpha))
...
class TestC2CIntegration(unittest.TestCase):
    ...
    def test_end_to_end_fusion_workflow(self):
        ...
        for i in range(4):
            self.assertEqual(fused_cache.keys[i].shape, tgt_cache.keys[i].shape)
