...
class LMModelState:
    ...
    def transition_to(self, new_mode: SleepMode) -> None:
        ...
        valid_transitions = {
            ...
            SleepMode.LIGHT_SLEEP: [SleepMode.AWAKE, SleepMode.DEEP_SLEEP, SleepMode.TRANSITIONING],
            SleepMode.DEEP_SLEEP: [SleepMode.LIGHT_SLEEP, SleepMode.TRANSITIONING],
            SleepMode.TRANSITIONING: [SleepMode.AWAKE, SleepMode.LIGHT_SLEEP, SleepMode.DEEP_SLEEP]
        }
    ...
    def prune_attention_heads(self, layer_id: str, head_ids: List[int]) -> None:
        ...
        self.attention_heads[layer_id]["active"] = [
            h for h in self.attention_heads[layer_id].get("active", [])
    ...
    def create_snapshot(self) -> StateSnapshot:
        ...
        state_str = f"{self.model_id}_{self.sleep_cycle_count}_{self.current_mode.value}"
    ...
    def save_state(self, filepath: Union[str, Path]) -> None:
        ...
        state_dict = {
            ...
            "gradients": {k: v.tolist() if isinstance(v, np.ndarray) else v 
    ...
    def load_state(cls, filepath: Union[str, Path]) -> "LMModelState":
        ...
        instance.gradients = {k: np.array(v) for k, v in state_dict["gradients"].items()}
        ...
        instance.last_transition_time = datetime.fromisoformat(state_dict["last_transition_time"])
