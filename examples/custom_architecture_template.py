...
from ai_sleep.sleep_controller import (
    AISleepController,
...
class CustomArchitectureController(AISleepController):
    ...
    def _execute_light_sleep(self) -> None:
        ...
        self.custom_strategies.optimize_custom_component_1(
            self.model_state, cache_threshold=0.3
        )
    ...
    def _execute_deep_sleep(self) -> None:
        ...
        self.custom_strategies.architecture_specific_pruning(
            self.model_state, pruning_ratio=0.1
        )
...
def main():
    ...
    controller = CustomArchitectureController(
        model_id="custom-vision-model-v1", enable_monitoring=True, auto_adapt=True
    )
    ...
    print(
        f"   ✓ Data entries after light sleep: {len(custom_state.custom_component_1)}"
    )
