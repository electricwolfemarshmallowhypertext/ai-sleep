...
def create_mock_kv_cache(
    n_layers: int = 12, n_heads: int = 12, seq_len: int = 32, head_dim: int = 64
):
...
def demo_c2c_fusion_mock():
    ...
    print(
        f"   ✓ Gate statistics: mean={stats['gate_stats']['mean']:.3f}, "
...
def demo_c2c_fusion_real():
    ...
    fuser = C2CFuser(
        n_layers=config_b["n_layers"], head_dim=config_b["head_dim"], rank=64
    )
    ...
    if cache_a.keys[0].shape[2] != cache_b.keys[0].shape[2]:
        print(
            f"   Note: Sequence lengths differ (A: {cache_a.keys[0].shape[2]}, "
